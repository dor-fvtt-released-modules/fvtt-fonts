...
export function notify (locMap={},type='info',locPlaceholders,options = {'permanent':false,'localize':true}) {
	const locString = loc(locMap.locDomain,locMap.locSection,locMap.locKey,locPlaceholders)
	ui.notifications.notify(`${constants.moduleTitle} | ${locString}`,type,{'permanent':options.permanent,'localize':options.localize});
	log(locString,type)
}
...
export function debug(debugString,force = false) {
	if (game.modules.get('_dev-mode')) {
		try {
			const isDebugging = game.modules.get('_dev-mode')?.api?.getPackageDebugValue(constants.moduleName);
